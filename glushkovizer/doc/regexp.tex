...
\begin{example}
    ...
    le langage \(L(E') = \{\varepsilon, b^* \cdot a\} \Leftrightarrow
    \{\varepsilon, a, b^+ \cdot a\}\).
...
Une des fonctions les plus importantes sur les expressions régulières est
\(flnf\) permettant de calculer \(FLNF(Exp(\Sigma))\) qui est un tuple défini
comme ceci~:
...
\begin{example}
    Prenons par exemple l'expression régulière suivante \(E = a \cdot b
    + c \cdot d\), avec \(E \in Exp(\mathbb{A})\) et \(\mathbb{A} = \{a, b, c,
    d\}\). Toujours à l'aide d'un arbre syntaxique, on peut calculer ce que
    \(flnf(E)\) donnerait.
    ...
    Il advient que \(flnf(E) = \{\{a, c\}, \{b, d\}, \varnothing, \delta\}\) avec
    \(\delta\) qui est défini comme ceci~:
...
\begin{example}
    ...
    (\ref{subsec:parenthese}) son calcul reste le même que si c'était une union.
...
des symboles du résultat \(E'\) avec \(linearization(E, n) = (E', m)\), seront
supérieurs ou égaux à \(n\) et inférieurs stricts à \(m\).

\begin{example}

    Si on reprend cette expression régulière \(E = \varepsilon + b^* \cdot a\),
    avec \(E \in Exp(\mathbb{A})\) et \(\mathbb{A} = \{a, b, c, d\}\).
