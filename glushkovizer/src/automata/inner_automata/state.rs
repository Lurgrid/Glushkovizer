    ...
    T: Eq + Hash + Clone,
{
    ...
    /// Returns current state outgoing transition symbols
    pub fn get_symbols_out(&self) -> Vec<T> {
        self.as_ref().follow.keys().cloned().collect()
    }

    /// Returns current state incoming transition symbols
    pub fn get_symbols_in(&self) -> Vec<T> {
        self.as_ref().previous.keys().cloned().collect()
    }

    /// Returns the number of outgoing transition symbols
    pub fn get_symbols_out_count(&self) -> usize {
        self.as_ref().follow.keys().len()
    }

    /// Returns the number of incoming transition symbols
    pub fn get_symbols_in_count(&self) -> usize {
        self.as_ref().previous.keys().len()
    }
